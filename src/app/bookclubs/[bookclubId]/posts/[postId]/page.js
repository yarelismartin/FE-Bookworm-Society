...
export default function PostDetail({ params }) {
  ...
  return (
    <div className="container">
      <article className="rounded-xl bg-white p-4  sm:p-6 lg:p-8 flex items-center justify-center mb-4">
        <div className="flex items-start sm:gap-8">
          <div className="hidden sm:grid sm:size-20 sm:shrink-0 sm:place-content-center sm:rounded-full sm:border-2 sm:border-indigo-500 overflow-hidden" aria-hidden="true">
            <img src={post.user?.imageUrl} alt="user" className="w-full h-full object-cover" />
          </div>
        ...
        <div key={comment.id} className="flex justify-center">
          <div className="relative grid grid-cols-1 gap-3 p-3 mb-8 border rounded-lg bg-white shadow-xl hover:shadow-2xl transition-shadow duration-300 w-full max-w-md">
            <div className="relative flex gap-4">
              <img src={comment.user?.imageUrl} className="relative rounded-lg -top-6 -mb-2 bg-white border h-16 w-16" alt="" loading="lazy" />
              <div className="flex flex-col w-full">
                <div className="flex flex-row justify-between">
                  <p className="relative text-lg whitespace-nowrap truncate overflow-hidden">{comment.user?.username}</p>
                </div>
                <p className="text-gray-400 text-xs">{formatDistanceToNow(new Date(post.createdDate), { addSuffix: true })}</p>
              </div>
            </div>
